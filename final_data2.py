...
def get_data(data):
    ...
    train_x1 = np.delete(train_x1, 0, axis=0)
    train_x2=train_x1
    train_x2 = np.delete(train_x2, 37, axis=1)
    train_x2 = np.delete(train_x2, 36, axis=1)
    train_x2 = np.delete(train_x2, 35, axis=1)
...
if __name__ == '__main__':
    ...
    data2_final="final_data_aver.csv"
    train_x=get_data(data2_final)


    train1=train_x[0:16263,:]
    test1=remove_outliars(train1)
    train2=train_x[16264:31240,:]
    test2 = remove_outliars(train2)
    train3=train_x[31241:47586,:]
    test3 = remove_outliars(train3)
    train4=train_x[47587:62360,:]
    test4 = remove_outliars(train4)
    train5=train_x[62361:,:]
    test5 = remove_outliars(train5)

    if r==1 :
        #with removal of outliars
        new_train=np.concatenate((test1,test2,test3,test4,test5),axis=0)

    else:
        #without removal of outliars
        new_train=np.concatenate((train1,train2,train3,train4,train5),axis=0)
    ...
    np.savetxt("final_data2_results.csv", arir, fmt='%f',delimiter=',')
